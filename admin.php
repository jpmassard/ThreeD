...
elseif (isset($_POST['save_settings'])) {
	...
	if(isset($_POST['unzipArchive']) && $_POST['unzipArchive'] == 'on') {
		$zip = new ZipArchive;
		...
		if ($res === TRUE) {
			if($zip->extractTo(dirname($file_path))) {
				...
				$query = 'UPDATE '.IMAGES_TABLE. ' SET isPano=1, representative_ext=\'jpg\' WHERE id='.$image_id;
				pwg_query($query);
				...
				if(count($files) > 0) {
					foreach($files as $file) {
						if(is_dir($dir . '/panos/' . $file)) {
							...
							if (!copy ($dir . '/panos/' . $file . '/thumb.jpg',
								$representative_file_path . '/' . $jpgpath)) {
								$errors['createThumb'] = l10n('Representative image creation failed');
							}
						}
					}
				}
				...
				$zip->close();
				$errors ['unzipfail'] = l10n('ZIP archive extraction failed');
			}
		} else {
			$errors ['nozipfile'] = l10n('couldn\'t open ZIP archive');
		}
	}
...
if('config' == $page['tab']) {
	...
	$image_id = $_GET['tab'];
	if($image_id == '') return;	// needed when two clicks on tabsheet... TODO
...
function extractPanorama ($path, $del) {
