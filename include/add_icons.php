...
function threed_add_icons ($pictures)
{
    ...
    for ($i=0; $i < count($pictures); $i++) 
    {
        ...
        $is3D = $pictures[$i]['is3D'] == 'true';

        if($conf['threed']['icon_2Dvideo'] and !$is3D and $is_video) $show_icons[$i][] = 'icon_2D';
        if($conf['threed']['icon_3Dvideo'] and $is3D and $is_video) $show_icons[$i][] = 'icon_3D';
        if($conf['threed']['icon_2Dphoto'] and !$is3D and $is_photo) $show_icons[$i][] = 'icon_2D';
    ...
    switch($conf['threed']['icon_position'])
    {
        ...
        case 'custom': $x = $conf['threed']['icon_xpos']; $y = $conf['threed']['icon_ypos']; break;
    }
