...
;';
  ...
  else
  {
    ...
    $file_path = $upload_dir.'/'.$filename_wo_ext.'.'.strtolower(get_extension($original_filename));
    $original_extension = strtolower(get_extension($original_filename));
    $file_path = $upload_dir.'/'.$filename_wo_ext.'.'.$original_extension;

    list($width, $height, $type) = getimagesize($source_filepath);
    if (in_array($original_extension, $conf['picture_ext']))
    {
    }
  ...
  $representative_ext=trigger_change('on_upload_done', $representative_ext, $original_extension, $file_path);
...
function do_tiff($rep_ext, $ext, $file_path)
{
  if ($ext != 'tif' or pwg_image::get_library() != 'ext_imagick')
     return $rep_ext;
...
function do_video ($rep_ext, $ext, $file_path)
{
 //
  ...
  if (!isset($ext) or !in_array($ext, $ffmpeg_video_exts))
    return $rep_ext;
...
function do_pdf ($rep_ext, $ext, $file_path)
{
   if (!isset($original_extension) or 'pdf' != $ext or pwg_image::get_library() != 'ext_imagick')
    return $rep_ext;
